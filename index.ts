...
const createNamesRouter = async (signingName: string, signatureVersion: number) => {
...
const getVerifiedUserAccount = async (userName: string, userPubkey: PublicKey) => {
  const nameRouterAccount = await getNameRouterAccount()
...
const checkRouter = async () => {
  const nameRouterAccount = await getNameRouterAccount()
  const data = await program.account.nameRouter.fetch(nameRouterAccount).catch(() => {
    return undefined
  });
...
const addUser = async (github_uid: string, user_public_key: string) => {
  ...
  const verifiedUserAccount = await getVerifiedUserAccount(userName, userPubkey)
  const data = await program.account.verifiedUser.fetch(verifiedUserAccount).catch(() => {
  ...
  const verifiedData = await program.account.verifiedUser.fetch(verifiedUserAccount)
...
export const addUserClaimAccount = async (username: string, repositoryAccount: PublicKey, amount: anchor.BN) => {
  const nameRouterAccount = await getNameRouterAccount()
...
app.post('/createUserMapping', async (req: Request, res: Response) => {
  ...
  if (req.body.pub_key === undefined || req.body.github_access_token === undefined || req.body.github_uid === undefined) {
  ...
  const success = await addUser(req.body.github_uid.toString(), req.body.pub_key.toString());
